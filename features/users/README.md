...
| `DuplicateUserFieldException` | Thrown by `ExposedUsersRepo.update`/`create` when PostgreSQL state `23505` or Xerial's exact SQLite UNIQUE/PRIMARYKEY result code identifies a username/email collision; propagates through `CacheUsersRepo` untouched; callers should catch and respond `409 Conflict`. |

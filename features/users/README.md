...
| `RegisteredUser` | Persisted entity: `id: UserId`, `username: Username`, `email: Email? = null`, `emailApproved: Boolean = false`. Approval belongs only to the current non-null email and is never exposed by the public listing. |
| `UsersFeatureUser` | Feature model returned by `UsersFeature.getAll` (client and server): `id: UserId`, `username: Username` — deliberately omits `email` and `emailApproved` (issue #67: the route is unauthenticated, so private `RegisteredUser` fields must never be exposed there). Mapper: `RegisteredUser.asUsersFeatureUser()`. Reverse mapper: `UsersFeatureUser.asRegisteredUser(email: Email?, emailApproved: Boolean)` — both omitted fields are explicit no-default arguments, so reconstruction cannot silently erase approval (Feature Interface Return Model Rule, both-directions bullet). |
| `ReadUsersRepo` | Read-only repo interface |
| `WriteUsersRepo` | Write-only repo interface, including conditional `approveEmail(id, expectedEmail)` for the current address only |
| `UsersRepo` | Combined CRUD repo interface |
| `CacheUsersRepo` | In-memory `FullCRUDCacheRepo` wrapper |
| `ExposedUsersRepo` | JVM/PostgreSQL-and-SQLite implementation (`users` table) |
...
- **Current-email approval:** `ExposedUsersRepo` stores a non-null `email_approved` column with a database default of `false`. Inserts always begin unapproved. A same-email update, including username-only changes, preserves approval; a changed, cleared, or absent email resets it to `false`. `approveEmail(id, expectedEmail)` updates only when both id and current stored email match, then mirrors the approved record into the cache and emits the normal update event. This prevents stale verification links from approving a replacement address.

...
| `DuplicateUserFieldException` | Thrown by `ExposedUsersRepo.update`/`create` when the Common-owned `dev.inmo.wishlist.features.common.common.isUniqueViolation` classifier identifies PostgreSQL state `23505` or Xerial's exact SQLite UNIQUE/PRIMARYKEY result code as a username/email collision; propagates through `CacheUsersRepo` untouched; callers should catch and respond `409 Conflict`. |

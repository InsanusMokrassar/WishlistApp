...
- **Email verification request** (`POST /email/requestMyEmailVerification`) — an authenticated owner requests a link for the exact current address; requires SMTP delivery and returns an explicit domain result rather than treating ordinary account states as transport errors.
...
| POST | `/email/requestMyEmailVerification` | Bearer (self) | `EmailVerificationRequest { expectedEmail: Email }` → `EmailVerificationRequestResult` | Sends or retries verification only for the caller's exact current address; result is one of `Sent`, `AlreadyApproved`, `Unavailable`, `NoEmail`, `EmailChanged`, or `DeliveryFailed` |
...
| `EmailFeature` (server) | `email/server` | Server-side interface: `isFeatureEnabled()`, `sendTestEmail(callerId, recipient)`, `setMyEmail(callerId, email?)`, `requestMyEmailVerification(callerId, expectedEmail)` — every caller-scoped method receives the authenticated `UserId` explicitly. Implemented by `EmailFeatureService`. |
| `EmailFeature` (client) | `email/client` | Client-side interface: `isFeatureEnabled()`, `sendTestEmail(recipient)`, `setMyEmail(email?)`, `requestMyEmailVerification(expectedEmail)` — caller identity is resolved server-side from the bearer token. Implemented by `KtorEmailFeature`. |
...
| `EmailVerificationRequest` / `EmailVerificationRequestResult` | `email/common` | Request body carrying the expected current email and the explicit non-exceptional outcome of an owner verification request. |
...
| `EmailVerificationAccountCoordinator` | `email/server` | Unconditional server singleton owning `UsersRepo`, `RolesRepo`, and one process-local mutex. Serializes full/username/self email mutations with current-email approval and invited-email equality checking plus guarded role promotion. |
...
| `EmailRegistrationInviteSender` | `email/server` | Auth registration hook that creates an email-bound verification deeplink and sends a labeled HTML anchor below validated `publicHttpOrigin`; returns a request-local compensation handle after successful delivery, removes the link on false/ordinary failure, and non-cancellably removes it before propagating cancellation. |
...
- **Storage, approval, and sending:** Per-user email storage (`PUT /email/myEmail`) is independent of SMTP and works through both `EmailFeatureService` and `DisabledEmailFeature`. Both implementations receive the same Koin-singleton `EmailVerificationAccountCoordinator`; no email service updates `UsersRepo` directly. The coordinator also serializes admin full replacements and username-only changes with verification. Repository mutation preserves approval only for an unchanged non-null email and resets it for a replacement or clear; verification conditionally approves only the exact current address before any role transition.
...
- **Client:** `KtorEmailFeature` is transport-only; no service wrapper needed (no memoization). Its capability probe and verification-request decode require a successful HTTP response, so a 4xx/5xx, network error, or malformed body reaches the caller as a recoverable failure rather than falsely reporting SMTP as disabled or delivery as `Sent`. The Boolean storage/update contracts remain status-based. Platform plugins (JS/JVM/Android) delegate to the shared `Plugin`.

...
- **Role requirement (issue #68):** this feature owns the `files.avatarChangeForOthers` gate. `Constants.avatarChangeForOthersFunctionalityId` (`files/common`) declares the `FunctionalityId`, and `files/server` `Plugin.setupDI` registers `FeatureRolesRegistry.Requirement(avatarChangeForOthersFunctionalityId, SuperAdminRole)` via `singleRequirement` — a gate's requirement lives in the feature it gates. `files/common` therefore `api`-depends on `roles/common`. The actual `PUT /files/avatar/{userId}` enforcement uses `RolesFeature.isFunctionalityAvailable` with the same functionality id, so registration and enforcement both use the same mechanism.

...
| POST | `/files/finalize` | Bearer | `FinalizeFileRequest` → `RegisteredFileMetaInfo \| 400` | Promote temporal upload to permanent storage; returns `RegisteredFileMetaInfo` (200) or `400` when the temp file is missing/expired or the payload is not an allowlisted raster image (see Architecture Notes); owner recorded from bearer principal |

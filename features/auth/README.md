...
| `AuthFeatureUser` | `@Serializable` private auth model returned by `getMe`/`getUser`/the "me" state flow: `id`, `username`, current `email`, and `emailApproved`. Pending email, requested-at, and cooldown deadline belong to the email-owned `EmailProfile` returned by `EmailFeature.getMyEmail`; auth remains the source of caller identity, credentials, and roles. |
...
- **Private identity projection:** `getMe`/`getUser` and the "me" state flow return `AuthFeatureUser` with identity, current email, and current-email approval only. `AuthFeatureService.getUser` uses a fresh identity read and never carries pending email, requested-at, or cooldown deadline. User-profile editing obtains the complete private email lifecycle through `EmailFeature.getMyEmail`; the public `UsersFeatureUser` listing remains username/id only and privacy boundaries are unchanged.

...
| `AuthFeatureUser` | `@Serializable` private owner model returned by `getMe`/`getUser`/the "me" state flow: `id`, `username`, current `email`, `emailApproved`, `pendingEmail`, and `emailChangeAllowedAt` (UTC epoch milliseconds). Only the authenticated owner's fresh private read carries lifecycle fields; public/privacy boundaries remain unchanged. |
...
- **Private lifecycle projection:** `getMe`/`getUser` and the "me" state flow return `AuthFeatureUser` with current, pending, approval, and deadline fields only on the authenticated owner's private surface. `AuthFeatureService.getUser` uses a fresh repository read for lifecycle truth; the public `UsersFeatureUser` listing remains username/id only and privacy boundaries are unchanged.

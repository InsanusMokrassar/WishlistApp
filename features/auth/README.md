...
| POST | `/auth/register` | None | `RegisterRequest → RegistrationResult \| 400 \| 415` | Creates an approved account or a pending required-email account; malformed bodies and unavailable registration, invalid fields, duplicate values, and failed invite delivery return 400; unsupported body/media can return Ktor's 415 |
| POST | `/auth/login` | None | `LoginRequest → AuthCredentials \| 400 \| 401 \| 415` | Validates credentials, returns token + refreshToken; malformed bodies return 400 and unsupported body/media can return Ktor's 415 |
| POST | `/auth/refresh` | None | `RefreshRequest → AuthCredentials \| 400 \| 401 \| 415` | Exchanges refreshToken for new credentials; malformed bodies return 400 and unsupported body/media can return Ktor's 415 |
...
- `AuthFeatureService.register` retains its legacy Kotlin string-length policy (8..72): too-short/empty passwords are refused, but this character count does not guarantee BCrypt's 72-byte UTF-8 limit. The email-authorized password-change flow separately requires at least eight Kotlin String units and at most 72 UTF-8 bytes without trimming; registration and root-only admin behavior remain unchanged.
...
- **Malformed Auth requests:** register, login, and refresh preserve Ktor's malformed-body HTTP 400 and applicable unsupported body/media HTTP 415 responses at the transport boundary; service-level outcomes remain their existing 400/401 contracts. Anonymous password-approval completion remains intentionally unauthenticated, and password completion does not mutate existing sessions or roles.

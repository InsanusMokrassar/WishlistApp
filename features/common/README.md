...
- **Web client root mount (`/`)** — the web client (JS single page application) is served from the
  ...
  static-folder key when only a single `staticFolder` is configured. The static routing configurator
...
- **Password-change browser navigation:** the client navigation adapter preserves
  `/password-change/{positiveUserId}/{canonical-v4-id}` through scaffold restoration and reload, and
  uses `/password-changed` only after the actionable node has been replaced. Malformed blank/extra
  segments are not normalized into an approval route; older user and wishlist parsing stays intact.
  Common itself gains no Auth/UI dependency for these feature-owned states.
- **Secret-safe shell and server logging:** the web shell places `<meta name="referrer"
  content="no-referrer">` before external resources. Server `CallLogging` uses the shared
  `safeCallLogLine` formatter, which emits only HTTP method and final status; request URLs, query,
  headers, bodies, exceptions, and redirect locations are intentionally absent from that log.

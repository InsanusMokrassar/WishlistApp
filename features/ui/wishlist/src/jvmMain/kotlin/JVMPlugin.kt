...
/**
 * JVM (Desktop) platform startup plugin for the wishlist UI feature.
 *
 * Delegates to [Plugin] for common DI and registers [NavigationNodeFactory] entries
 * for all four JVM Compose-Desktop views.
 */
object JVMPlugin : StartPlugin {
    override fun Module.setupDI(config: JsonObject) {
        ...
        singleWithRandomQualifier<NavigationNodeFactory<ViewConfig>> {
            NavigationNodeFactory.Typed<WishlistEditViewConfig, ViewConfig> { chain, cfg ->
                WishlistEditView(
                    chain,
                    cfg
                )
        ...
        singleWithRandomQualifier<NavigationNodeFactory<ViewConfig>> {
            NavigationNodeFactory.Typed<WishlistItemEditViewConfig, ViewConfig> { chain, cfg ->
                WishlistItemEditView(
                    chain,
                    cfg
                )
            }

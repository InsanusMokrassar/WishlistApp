 ...
 * activity starts — to the coroutine-based [pickImageFile] expect declaration.
 *
 ...
 * pipeline can treat it as a regular `java.io.File` (the Android `MPPFile`).
 */
object AndroidImagePicker {
    ...
    private fun copyToTempFile(context: Context, uri: Uri): File? {
        ...
        val extension = mime
            ...
            ?: "img"
        val target = File.createTempFile("image_${uuid4()}", ".$extension")
        target.deleteOnExit()
...
/**
 * Android image picker: delegates to [AndroidImagePicker], which `MainActivity` wires up.
 *
 * @return The chosen image as an [MPPFile], or `null` when cancelled / picker unavailable.
 */
actual suspend fun pickImageFile(): MPPFile? = AndroidImagePicker.pick()

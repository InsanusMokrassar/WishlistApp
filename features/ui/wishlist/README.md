...
- **Wishlist detail item sorting:**
  - `WishlistViewModel` exposes `sortModeState: StateFlow<WishlistSortMode>` and `sortedItemsState: StateFlow<List<RegisteredWishlistItem>>` (derived via `combine(itemsState, sortModeState)`), plus `onSortModeSelected(mode)`.
  - The view renders the same sort-button selector as the all-items screen (one button per `WishlistSortMode`), but the `None` button is labelled `sortDefault` ("Default") instead of `sortNone` ("Grouped") since the detail screen has a single wishlist and no groups. `None` keeps the stored item order; `Cost`/`Priority`/`Title` reuse the same orderings as the all-items screen.
  - The sort selector is now a reusable per-platform `WishlistSortSelector(selected, onSortModeSelected, noneLabel)` composable (jvm/js/android) shared by `WishlistView` and `UserWishlistsView`; the detail screen passes `noneLabel = WishlistStrings.sortDefault` while the all-items screen uses the default `noneLabel = WishlistStrings.sortNone`.

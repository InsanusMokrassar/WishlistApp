...
- **All-items view:**
  ...
  - In the grouped (`None`) presentation, each wishlist group header (the wishlist title) is clickable: tapping it calls `viewModel.onWishlistSelected(wishlist)` → `interactor.onWishlistSelected(node, wishlistId)`, implemented in `ClientPlugin` to push `WishlistViewConfig(wishlistId)` (opens the wishlist detail screen). JS renders the header with `text-primary` + pointer cursor; JVM/Android add `Modifier.clickable` to the header `Text`.

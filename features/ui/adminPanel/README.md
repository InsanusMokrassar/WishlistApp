...
- Single `AdminPanelModel` interface consumed by all 8 ViewModels; implemented by `DefaultAdminPanelModel` in the common UI package and registered as an interface `single` in `Plugin.kt`. Its constructor receives `AdminFeature`, `EmailFeature`, and `AuthCredentialsStorage`.
...
- **Dashboard users:** `AdminPanelViewModel` loads `AdminPanelModel.getAllUsers()` on initial display, resume, and authenticated-session restoration. Refresh work is launched separately from the authorization collector, so logout immediately cancels and clears a suspended request; request versions still suppress late or older responses. Dashboard lifecycle state, request versions/jobs, authorization checks, and publications are confined to a `Dispatchers.Main.immediate` child scope that retains the navigation ViewModel lifecycle job; UI callbacks use that dispatcher and deterministic tests inject one shared serial dispatcher. The dashboard exposes separate loading, empty, and retryable failure states. Selecting a row uses the dashboard interactor to push `AdminUserViewConfig(userId)` directly.
...
- **Feature Interface Return Model Rule:** `AdminPanelModel` and its ViewModels' state flows now hold `AdminUser`/`AdminWishlist`/`AdminWishlistItem` (from `admin.common.models`) instead of the `users`/`wishlist` features' `RegisteredUser`/`RegisteredWishlist`/`RegisteredWishlistItem` persistence entities — `DefaultAdminPanelModel` and `AdminUsersListViewModel`/`AdminUserViewModel`/`AdminWishlistsListViewModel`/`AdminWishlistViewModel`/`AdminWishlistEditViewModel`'s state flows were retyped accordingly. Field access (`.username`, `.title`, `.email`, etc.) is unchanged since the new models mirror the old ones' display fields.

...
/** Android Compose-Material3 view for the admin user detail screen. */
class AdminUserView(
        ...
        Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
            ...
            if (loading) {
                ...
                Row(
                    ...
                    Text(AdminPanelStrings.userWishlistsSection.translation(resources), style = MaterialTheme.typography.titleLarge)
                    Button(onClick = { viewModel.onAddWishlist() }) { Text(AdminPanelStrings.addWishlistForUserButton.translation(resources)) }
                }
                Spacer(modifier = Modifier.height(8.dp))
                if (wishlists.isEmpty()) {
                    Text(AdminPanelStrings.noWishlistsForUser.translation(resources))
                } else {
                        ...
                        items(wishlists) { wishlist ->
                            ListRow(onSelect = { viewModel.onOpenWishlist(wishlist.id) }) {
                                Row(
                                    ...
                                    Text(wishlist.title)
                                    Text("#${wishlist.id.long}", color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.5f))
                                }
                            }
                        }
                    }

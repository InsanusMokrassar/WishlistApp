...
- **Profile edit gating** (`UserEditViewModel`):
  ...
  - **Logout exit (issue #53):** `UserEditViewModel` subscribes to `UsersModel.currentUserIdFlow` via `Flow<T?>.subscribeOnLoggedOut` (`features/common/client` `utils/SubscribeOnLoggedOut.kt`); on a logout transition it calls `interactor.onNavigateBack(node)`, popping the edit screen so the underlying (non-edit) profile view is revealed for the now-anonymous caller.

...
| `UsersModel` | Single feature model (renamed from `UsersListModel`). Wraps `UsersFeature.getAll()` (returns `UsersFeatureUser` — no email, see `features/users/README.md`), auth "me" `StateFlow<RegisteredUser?>` from `features/auth/client` `Scope.meStateFlow` (`getCurrentUserId`, `isCurrentUserRootFlow` — now SuperAdmin-role-backed, see Architecture Notes), admin `AdminFeature.usersManagement` (`updateUsername`, `setPassword`, `deleteUser`), and `FilesClientService` (`getAvatar`, `uploadAvatar`, `imageUrl`, `loadImageBytes`); `getUser(id)` resolves a `UsersFeatureUser?` from the public list |

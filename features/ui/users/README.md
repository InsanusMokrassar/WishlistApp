...
- **Contextual Back navigation (issue #43):** `UserView` Back is now REPLACE semantics: `UserViewInteractor.onBack` does `node.chain.replace(node, UserWishlistsViewConfig(userId))` (navigate to that user's all-items screen) instead of pop. `UserViewModel` exposes `backLabelState: StateFlow<String?>` derived from the loaded `userState` (= `username.string`); the view renders `backLabel ?: UsersListStrings.backButton.translation()`.

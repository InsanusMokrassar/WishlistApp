...
| `PasswordChangeViewModel` / `PasswordChangeViewInteractor` | Shared token-authorized form state and client navigation delegate. The ViewModel stores password/confirmation only in memory; the interactor replaces a confirmed pending node with `Completed` and leaves through the users-list route. |

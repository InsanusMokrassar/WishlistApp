...
- **Profile edit gating** (`UserEditViewModel`):
  ...
  - **Owner email verification:** the editor derives `canManageOwnEmailState` from the current caller id and the server capability probe. It fetches `ClientAuthFeature.getMe()` only for that owner; root editing someone else never sees the private email controls. With no stored email, save first calls `setMyEmail` and only requests verification after persistence succeeds. A saved unapproved address is read-only and offers resend; an approved address has no resend. Every save/retry refreshes the private profile so a deeplink approval is reflected, while an unsaved draft is preserved. UI surfaces `Sent`, `AlreadyApproved`, `Unavailable`, `NoEmail`, `EmailChanged`, and `DeliveryFailed` separately; invalid input, load failure, and failed storage are separate local errors. The JS, JVM, and Android implementations use the same state and controls.

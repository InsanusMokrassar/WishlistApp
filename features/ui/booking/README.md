...
| Type | Description |
|------|-------------|
| `BookingModel` | Interface wrapping `features/booking/client` `BookingFeature`; methods `getBookingState(itemId)`, `bookItem(itemId)` → `BookingFeature.tryBook`, `cancelBooking(itemId)` → `BookingFeature.cancelBooking`, `myPresents()`. Single anonymous impl in the feature's common `Plugin.kt`. |
...
- View A (`BookingView`) is compact and embedded inline — it has NO top-bar title and NO back button. View B (`MyPresentsView`) still implements `TopBarTitleProvider` and reuses the shared `BackButton` / `ListRow` / `ScreenTitle` components.

...
- Security note: this only closes a UX/discoverability gap. Client-side hiding is not a new
  ...
  `AdminRoutingsConfigurator`'s server-side SuperAdmin-role check (via `roles.server`'s
  `RolesFeature.isFunctionalityAvailable(callerId, adminPanelFunctionalityId)`, issue #68 — replaced the
  previous inline `rootUsername` comparison), matching the pattern already relied on by
  `UserEditViewModel.isRootState`.

...
| `AuthModel` | Interface: `isAlreadyLoggedIn()`, `login(username, password): Boolean`, `logout()`, `userAuthorisedState: StateFlow<Boolean>`, `getConfig(): AuthConfig`, `isRegistrationEnabled(): Boolean`, `register(username, password, email?): RegistrationResult?` |
| `AuthViewInteractor` | Interface: `onUserLoggedIn(node)`, `onUserLoggedOut()` — implemented in `client/ClientPlugin` |
| `AuthViewModel` | Holds `usernameState`, `passwordState`, `emailState`, `loadingState`, `errorState`, `formExpandedState`, `registerModeState`, `pendingEmailVerificationState`, `registrationEnabledState`, `requireEmailForRegistrationState`, `loggedInState`, `loginEnabledState`; methods include `onDismissPendingEmailVerification()` alongside form, login, registration, and logout actions. |
...
- An `authorized` registration clears and closes the form before transitioning to logged-in state. A `pendingEmailVerification` registration also clears and closes the form, but stays logged out and shows a separate localized check-email confirmation. JS renders it with Calm Studio modal components; JVM and Android use their existing Compose `Dialog`/`Surface` conventions. Dismissal only hides the confirmation, and opening either form clears stale confirmation state.

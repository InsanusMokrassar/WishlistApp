...
- **`AuthModel`** (commonMain interface) exposes authentication state and login, logout, configuration, and registration operations. `DefaultAuthModel` is the mandatory named implementation in a separate commonMain file; its primary constructor stores `ClientAuthFeature` and `AuthCredentialsStorage` as `private val` dependencies. The feature's common `Plugin.kt` binds it explicitly: `single<AuthModel> { DefaultAuthModel(authFeature = get(), credentialsStorage = get()) }`. Server URL storage remains owned by `features/ui/serverUrl`.

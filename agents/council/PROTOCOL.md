...
Architecture coordinates the council inside Architecturing; the council [Architect](roles/ARCHITECT.md) performs the substantive architecture research, design and test planning. The outer coordinator collates participant-authored content and manages the process, not its own technical solution. Participants apply only [COMMON](COMMON.md), this protocol, [MODELS](MODELS.md), and their assigned role contract.

...
| **Medium** | Logic gap or missing requirement that does not break the feature | Report to Orchestrator in step report; Orchestrator decides |
| **High** | Functional defect or architectural violation that breaks correctness | Restart full cycle from Planning (step 1) |
| **Critical** | Security issue, data loss risk, or unresolvable contradiction | Stop immediately; ask operator via GitHub issue comment |
...
- If escalated to Critical → stop and ask operator via GitHub issue comment.
- Record the escalation in the current step report with the cycle count.

## Process

* Role must skip all the step numbers up to the last one when the validator did its work, including its step number. For example, if there are steps 000, 001, 002, 003, 004 and last time validator worked on step 003, it must skip 000, 001, 002, 003 and start its analysis from step 004.

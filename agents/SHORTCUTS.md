...
2. Task-specific:
    * root (if there is no direct role specified) → `agents/ORCHESTRATOR.md` + (`agents/local.ORCHESTRATOR.md` (if exists)) (priorities of agents: `sonnet` / `opus`). **ROOT MUST NEVER DO ALL WORK BY ITSELF - ONLY WORK OF ORCHESTRATOR. ROOT IS THE ENTRY POINT ONLY — it receives the task, creates the task folder, places PROMPT.md, and spawns the first Planning subagent. It has no direct role propagation and must not perform planning, architecture, coding, or validation itself.**

...
2. Task-specific:
    * root (if there are no direct saying of role) → `agents/ORCHESTRATOR.md` + (`agents/local.ORCHESTRATOR.md` (if exists)) (priorities of agents: `sonnet` / `opus`). **ROOT MUST NEVER DO ALL WORK BY ITSELF - ONLY WORK OF ORCHESTRATOR**

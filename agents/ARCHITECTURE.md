...
Architecture is the small orchestrator of the expert council, inside the existing Architecturing stage. It receives accepted Planning, coordinates the agents described by [PROTOCOL](PROTOCOL.md), and returns one normal `<NNN>-architecturing.md` report. It does not implement code, run Verification, or replace [Validation](VALIDATOR.md). Prefer HL; use ML only as the documented fallback.

## Council orchestration

1. Read Planning and the applicable repository evidence from [PROJECT_ARCHITECTURE](PROJECT_ARCHITECTURE.md) and [COUNCIL_ADAPTER](COUNCIL_ADAPTER.md). Collect required research before freezing the common brief; record sources and ask the root to relay unresolved operator questions.
...
3. Using the host's native subagent tools, launch one fresh independent subagent per discovered role for proposals, **in parallel**. Give all agents identical frozen evidence and a private allocated output, with only the active role and output assignment differing. Start the whole wave before waiting; do not expose sibling results during the wave.
...
7. Publish the final Architecturing report with the accepted design, test specifications and README delta included in full. Copy the accepted implementation sections unchanged; add only the council outcome and evidence references. Return that report to root for Coding. Detailed validation of requirements and council comments belongs to the later Validation stage.
...
- Rationale, considered alternatives, risks, assumptions, compatibility, migration and rollout/rollback effects.
...
- A `## Council outcome` section with the brief, accepted candidate, complete roster, same-cycle votes, remaining nonblocking notes and terminal state.
...
Only `CONSENSUS` is a successful Architecturing result. `NEEDS_INFORMATION`, `IRRECONCILABLE`, and `PROCESS_FAILURE` return an explicit blocked report and cannot enable Coding. Full requirements, implementation and comment-resolution auditing remains [Validation's responsibility](VALIDATOR.md).

## Role requirement placement

Existing feature documentation points here for the project rule. Its unchanged definition now lives in [PROJECT_ARCHITECTURE.md — Role requirement placement](PROJECT_ARCHITECTURE.md#role-requirement-placement).
